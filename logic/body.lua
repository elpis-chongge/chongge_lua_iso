...
function Body:drawaabb()
  local box = self.aabb
  local minX, maxX = box[1], box[3]
  local minY, maxY = box[2], box[4]
  local color = self.sleeping and Color.blue or Color.red
  drawline(Vector3(minX, minY), Vector3(maxX, minY), color)
  drawline(Vector3(maxX, minY), Vector3(maxX, maxY), color)
  drawline(Vector3(maxX, maxY), Vector3(minX, maxY), color)
  drawline(Vector3(minX, maxY), Vector3(minX, minY), color)
end
