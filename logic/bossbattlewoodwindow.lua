...
function BossBattleWoodWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.BossBattleWoodWindow.package, WinResConfig.BossBattleWoodWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetBossDungeonWood_BossBattleWindowUis(contentPane)
...
function BossBattleWoodWindow.UpdateTextDisplay()
...
function BossBattleWoodWindow.LoadChangeEffect()
  ...
  LuaUtil.ChangeBadgeColorProperty(changeEffect, "_DissolutionLineColor", 35, 80, 49, 255, 0)
...
function BossBattleWoodWindow.ReleaseTexture()
...
function BossBattleWoodWindow.UpdateInfo(notChange)
  bossSpineName = ""
  ...
  if chapterData then
        ...
        item.onClick:Set(function()
          if list.selectedIndex ~= i then
            ...
            switchTrans:SetHook("sign", function()
              ...
              BossBattleWoodWindow.ShowStateInfo(stateData)
            end)
            switchTrans:Play()
    ...
    if openIndex and nil == notChange then
      ...
      BossBattleWoodWindow.ShowStateInfo(allStateData[openId])
    end
...
function BossBattleWoodWindow.ShowStateInfo(stateData)
  if stateData then
...
function BossBattleWoodWindow.ShowTips()
  BossDungeonMgr.InitAssetsTips(uis.Main.AssetsTipsList, {
    COMMON_ITEM_ID.ENERGY
  })
end

function BossBattleWoodWindow.StartBattle()
...
function BossBattleWoodWindow.AiBattleOnClick()
  if BossDungeonMgr.SufficientQuantity(curStageData.cost) then
    OpenWindow(WinResConfig.SweepWindow.name, nil, curStageData.id)
  end
end

function BossBattleWoodWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.BossBattleWoodWindow.name, uis.Main.CurrencyReturn)
  local bigBtn = uis.Main.Info.ButtonRegion.Star2Btn
  bigBtn.onClick:Set(BossBattleWoodWindow.StartBattle)
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_wood_big.prefab", bigBtn:GetChild("EffectHolder"))
  local centreBtn = uis.Main.Info.ButtonRegion.Star1Btn
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_wood_centre.prefab", centreBtn:GetChild("EffectHolder"))
  centreBtn.onClick:Set(BossBattleWoodWindow.StartBattle)
  local dispatchBtn = uis.Main.Info.ButtonRegion.DispatchBtn
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_wood_small.prefab", dispatchBtn:GetChild("EffectHolder"))
  dispatchBtn.onClick:Set(BossBattleWoodWindow.AiBattleOnClick)
end

function BossBattleWoodWindow.OnShown()
  if uis then
    BossBattleWoodWindow.ShowTips()
    BossBattleWoodWindow.UpdateInfo(true)
  end
end

function BossBattleWoodWindow.OnClose()
  uis = nil
  contentPane = nil
  if jumpTb then
    jumpTb.Close()
