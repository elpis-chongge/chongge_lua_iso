...
function BossBattleWaterWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.BossBattleWaterWindow.package, WinResConfig.BossBattleWaterWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetBossDungeonWater_BossBattleWindowUis(contentPane)
...
function BossBattleWaterWindow.UpdateTextDisplay()
...
function BossBattleWaterWindow.LoadChangeEffect()
  ...
  LuaUtil.ChangeBadgeColorProperty(changeEffect, "_DissolutionLineColor", 33, 92, 123, 255, 0)
...
function BossBattleWaterWindow.ReleaseTexture()
...
function BossBattleWaterWindow.UpdateInfo(notChange)
  bossSpineName = ""
  ...
  if chapterData then
        ...
        item.onClick:Set(function()
          if list.selectedIndex ~= i then
            ...
            switchTrans:SetHook("sign", function()
              ...
              BossBattleWaterWindow.ShowStateInfo(stateData)
            end)
            switchTrans:Play()
    ...
    if openIndex and nil == notChange then
      ...
      BossBattleWaterWindow.ShowStateInfo(allStateData[openId])
    end
...
function BossBattleWaterWindow.ShowStateInfo(stateData)
  if stateData then
...
function BossBattleWaterWindow.ShowTips()
  BossDungeonMgr.InitAssetsTips(uis.Main.AssetsTipsList, {
    COMMON_ITEM_ID.ENERGY
  })
end

function BossBattleWaterWindow.StartBattle()
...
function BossBattleWaterWindow.AiBattleOnClick()
  if BossDungeonMgr.SufficientQuantity(curStageData.cost) then
    OpenWindow(WinResConfig.SweepWindow.name, nil, curStageData.id)
  end
end

function BossBattleWaterWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.BossBattleWaterWindow.name, uis.Main.CurrencyReturn)
  local bigBtn = uis.Main.Info.ButtonRegion.Star2Btn
  bigBtn.onClick:Set(BossBattleWaterWindow.StartBattle)
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_water_big.prefab", bigBtn:GetChild("EffectHolder"))
  local centreBtn = uis.Main.Info.ButtonRegion.Star1Btn
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_water_centre.prefab", centreBtn:GetChild("EffectHolder"))
  centreBtn.onClick:Set(BossBattleWaterWindow.StartBattle)
  local dispatchBtn = uis.Main.Info.ButtonRegion.DispatchBtn
  UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/BossButtonCutaway/FX_ui_BossButtonCutaway_water_small.prefab", dispatchBtn:GetChild("EffectHolder"))
  dispatchBtn.onClick:Set(BossBattleWaterWindow.AiBattleOnClick)
end

function BossBattleWaterWindow.OnShown()
  if uis then
    BossBattleWaterWindow.ShowTips()
    BossBattleWaterWindow.UpdateInfo(true)
  end
end

function BossBattleWaterWindow.OnClose()
  uis = nil
  contentPane = nil
  if jumpTb then
    jumpTb.Close()
