...
function Activity2PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity2PassportWindow.package, WinResConfig.Activity2PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1001_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity2PassportWindow.name, {true})
      if passInfo then
        Activity2PassportWindow.InitPassport()
        Activity2PassportWindow.CheckItemTime()
      end
...
function Activity2PassportWindow.UpdateTextDisplay()
...
function Activity2PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity2PassportWindow.name,
        com = item,
...
function Activity2PassportWindow.InitEffect()
...
function Activity2PassportWindow.InitLv()
...
function Activity2PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity2PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity2PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity2PassportWindow.SaveLevel()
  local tb = {}
...
function Activity2PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity2PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity2PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity2PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity2PassportWindow.ShowReward()
  Activity2PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity2PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity2PassportWindow.ShowLevelUp()
end

function Activity2PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity2PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity2PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity2PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity2PassportWindow.RefreshRewardEnd(lv1, Activity2PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity2PassportWindow.RefreshRewardEnd(lv2, Activity2PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity2PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity2PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity2PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity2PassportWindow.name)
...
function Activity2PassportWindow.RefreshUI()
  Activity2PassportWindow.GetExpMxp()
  Activity2PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity2PassportWindow.GetExpMxp()
...
function Activity2PassportWindow.SetLevelInfo()
...
function Activity2PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity2PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity2PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity2PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity2PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity2PassportWindow.RefreshRewardShow(data, objReward)
      Activity2PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity2PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity2PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity2PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity2PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity2PassportWindow.IsReward(passId, lv)
...
function Activity2PassportWindow.RefreshRewardFirst()
  Activity2PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity2PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity2PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity2PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity2PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity2PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity2PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity2PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity2PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity2PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity2PassportWindow.RefreshRewardShow(data, Item)
...
function Activity2PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity2PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity2PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity2PassportWindow.name)
end

function Activity2PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity2PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity2PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity2PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity2BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity2PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity2PassportWindow.InitTask()
...
function Activity2PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity2PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity2PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity2PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity2PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity2PassportWindow.name) then
        local lvUp = Activity2PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity2PassportWindow.ShowExpBarAnim(para)
    else
      Activity2PassportWindow.SetLevelInfo()
    end
...
function Activity2PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity2PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity2PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity2PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity2PassportWindow
