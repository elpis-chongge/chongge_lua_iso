...
function Activity2ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity2ChallengeWindow.package, WinResConfig.Activity2ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1001_ChallengeWindowUis(contentPane)
...
function Activity2ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity2ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity2ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity2ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity2ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity2ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity2ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity2ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity2ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity2ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity2ChallengeWindow.ShopInfo()
...
function Activity2ChallengeWindow.GetChapterPassById(id)
...
function Activity2ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity2ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity2ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity2ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity2ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity2ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1624))
end

function Activity2ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity2ChallengeWindow.name)
end

function Activity2ChallengeWindow.OnShown()
  if uis then
    Activity2ChallengeWindow.UpdateInfo()
  end
end

function Activity2ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity2ChallengeWindow
