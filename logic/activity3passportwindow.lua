...
function Activity3PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3PassportWindow.package, WinResConfig.Activity3PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity3PassportWindow.name, {true})
      if passInfo then
        Activity3PassportWindow.InitPassport()
        Activity3PassportWindow.CheckItemTime()
      end
...
function Activity3PassportWindow.UpdateTextDisplay()
...
function Activity3PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity3PassportWindow.name,
        com = item,
...
function Activity3PassportWindow.InitEffect()
...
function Activity3PassportWindow.InitLv()
...
function Activity3PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity3PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity3PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity3PassportWindow.SaveLevel()
  local tb = {}
...
function Activity3PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity3PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity3PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity3PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity3PassportWindow.ShowReward()
  Activity3PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity3PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity3PassportWindow.ShowLevelUp()
end

function Activity3PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity3PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity3PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity3PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity3PassportWindow.RefreshRewardEnd(lv1, Activity3PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity3PassportWindow.RefreshRewardEnd(lv2, Activity3PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity3PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity3PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity3PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity3PassportWindow.name)
...
function Activity3PassportWindow.RefreshUI()
  Activity3PassportWindow.GetExpMxp()
  Activity3PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity3PassportWindow.GetExpMxp()
...
function Activity3PassportWindow.SetLevelInfo()
...
function Activity3PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity3PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity3PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity3PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity3PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity3PassportWindow.RefreshRewardShow(data, objReward)
      Activity3PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity3PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity3PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity3PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity3PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity3PassportWindow.IsReward(passId, lv)
...
function Activity3PassportWindow.RefreshRewardFirst()
  Activity3PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity3PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity3PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity3PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity3PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity3PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity3PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity3PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity3PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity3PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity3PassportWindow.RefreshRewardShow(data, Item)
...
function Activity3PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity3PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity3PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3PassportWindow.name)
end

function Activity3PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity3PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity3PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity3BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity3PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity3PassportWindow.InitTask()
...
function Activity3PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity3PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity3PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity3PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity3PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity3PassportWindow.name) then
        local lvUp = Activity3PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity3PassportWindow.ShowExpBarAnim(para)
    else
      Activity3PassportWindow.SetLevelInfo()
    end
...
function Activity3PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity3PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity3PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity3PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity3PassportWindow
