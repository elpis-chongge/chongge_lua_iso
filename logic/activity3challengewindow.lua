...
function Activity3ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3ChallengeWindow.package, WinResConfig.Activity3ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_ChallengeWindowUis(contentPane)
...
function Activity3ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity3ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity3ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity3ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity3ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity3ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity3ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity3ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity3ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity3ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity3ChallengeWindow.ShopInfo()
...
function Activity3ChallengeWindow.GetChapterPassById(id)
...
function Activity3ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity3ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity3ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity3ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity3ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1624))
end

function Activity3ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3ChallengeWindow.name)
end

function Activity3ChallengeWindow.OnShown()
  if uis then
    Activity3ChallengeWindow.UpdateInfo()
  end
end

function Activity3ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity3ChallengeWindow
